...
const DemoApp = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://calac.cafe24app.com:8001/scheduler/category?currentUserNo=${session.userInfo.no}`
      )
      .then((response) => {
  ...
  function handleModalSubmit() {
    ...
    axios
      .post("http://calac.cafe24app.com:8001/scheduler/insert", {
  ...
  function handleEventChange(changeInfo) {
    axios // 새 이벤트 DB에 UPDATE
      .put(`http://calac.cafe24app.com:8001/scheduler/update/${changeInfo.event.id}`, {
  ...
  function handleEditModalSubmit() {
    ...
    axios // 새 이벤트 DB에 UPDATE
      .put(`http://calac.cafe24app.com:8001/scheduler/update/${updatedEvent.id}`, {
        title: title,
        start: start,
        end: end,
        color: color,
  ...
  const handleEventDelete = () => {
    if (
      ...
      axios // 새 이벤트 DB에 DELETE
        .delete(`http://calac.cafe24app.com:8001/scheduler/delete/${updatedEvent.id}`)
        .then(() => {
          // 성공시 UI에도 바로 반영
  ...
  const handleAddCategory = () => {
    ...
    axios // DB에 INSERT
      .post("http://calac.cafe24app.com:8001/scheduler/category/insert", {
  ...
  const handleDeleteCategory = (option) => {
    if (
      ...
      axios // DB에서 카테고리 DELETE
        .delete(`http://calac.cafe24app.com:8001/scheduler/category/delete/${option.id}`)
        .then(() => {
          // Remove event from calendar
      ...
      const url = `http://calac.cafe24app.com:8001/scheduler/event/color/delete/${encodeURIComponent(
        option.value
      )}`;
  ...
  const updateColor = (option) => {
    axios // DB에 카테고리 UPDATE
      .put(`http://calac.cafe24app.com:8001/scheduler/category/update/${option.id}`, {
        value: pickedColor,
        label: option.label,
      })
      .then((response) => {
    ...
    const url = `http://calac.cafe24app.com:8001/scheduler/event/color/update/${encodeURIComponent(
      option.value
    )}`;
